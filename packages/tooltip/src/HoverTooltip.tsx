...
import { ChartCanvasContext, first, GenericComponent, isDefined, last } from "@reincharts/core";
...
const defaultBackgroundShapeCanvas = (
    ...
    const {
        toolTipFillStyle,
        toolTipStrokeStyle = HoverTooltip.defaultProps.toolTipStrokeStyle,
        toolTipStrokeWidth = HoverTooltip.defaultProps.toolTipStrokeWidth,
    } = props;
    ...
    ctx.lineWidth = toolTipStrokeWidth;
    ctx.stroke();
};
...
const defaultOrigin = (props: HoverTooltipProps, moreProps: any, bgSize: any, pointWidth: any) => {
    ...
    const { mouseXY, xAccessor, currentItem, xScale, chartConfigs, width } = moreProps;
    ...
    if (isDefined(chartId) && isDefined(yAccessor) && isDefined(chartConfigs) && isDefined(chartConfigs.findIndex)) {
        const yValue = yAccessor(currentItem);
        const chartIndex = chartConfigs.findIndex((c: any) => c.id === chartId);

        y = Math.round(chartConfigs[chartIndex].yScale(yValue));
    }
...
export interface HoverTooltipProps {
    ...
    readonly toolTipStrokeWidth?: number;
