...
export interface RenkoOptions {
    readonly reversalType: string;
    readonly fixedBrickSize: number;
...
export default function () {
    let options: RenkoOptions = defaultOptions;
    ...
    const calculator = (rawData: any[]) => {
        ...
        const volumeSource = path(volumePath);
        ...
        const source =
            sourcePath === "high/low"
                ? (d: any) => ({ high: highSource(d), low: lowSource(d) })
                : (d: any) => ({ high: closeSource(d), low: closeSource(d) });
        ...
        rawData.forEach(function (d, idx) {
            if (brick.from === undefined) {
                brick.high = highSource(d);
                brick.low = lowSource(d);
            ...
            brick.volume = (brick.volume || 0) + volumeSource(d);
            ...
            // @ts-ignore
            brick.high = Math.max(brick.high, highSource(d));
            // @ts-ignore
            brick.low = Math.min(brick.low, lowSource(d));
