...
export default function () {
    let options: PointAndFigureOptions = defaultOptions;
    ...
    const calculator = (rawData: any[]) => {
        ...
        const source =
            ...
            sourcePath === "high/low" ? ((d: any) => ({ high: highSource(d), low: lowSource(d) })) : ((d: any) => ({ high: closeSource(d), low: closeSource(d) }));
        ...
        rawData.forEach(function (d) {
            // @ts-ignore
            column.volume = (column.volume || 0) + volumeSource(d);
