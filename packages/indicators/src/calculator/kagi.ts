...
export interface KagiOptions {
    ...
    readonly sourcePath: string; // close
...
export default function () {
    let options: KagiOptions = defaultOptions;
    ...
    const calculator = (data: any[]) => {
        ...
        const source = path(sourcePath); // close
        ...
        data.forEach(function (d) {
            if (line.from === undefined) {
                ...
                if (!line.open) {
                    line.open = openSource(d);
                }
                line.high = highSource(d);
                line.low = lowSource(d);
                if (!line.close) {
                    line.close = source(d);
                }
            ...
            line.volume = (line.volume || 0) + volumeSource(d);
            // @ts-ignore
            line.high = Math.max(line.high, highSource(d));
            // @ts-ignore
            line.low = Math.min(line.low, lowSource(d));
