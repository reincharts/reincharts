...
export function getDimensions({ width, height }: any, chartProps: any) {
    const chartHeight = functor(chartProps.height)(height) || height;
...
export function getNewChartConfig(innerDimension: any, children: any, existingChartConfig: ChartConfig[] = []) {
    return React.Children.map(children, (each): ChartConfig | undefined => {
        ...
        return {
            id,
            origin: functor(origin)(width, availableHeight, height),
            padding,
...
/**
 * Determines which chart panels are under the mouse pointer by testing
 * whether the mouse Y-coordinate lies within each chart's vertical bounds.
 *
 * @param chartConfig  Array of ChartConfig objects, each with an origin [x, y] and height.
 * @param mouseXY      Tuple [x, y] of the current mouse position in pixels.
 *
 * @returns            Array of chart IDs whose regions contain the mouse position.
 */
