...
export interface EdgeCoordinateProps {
    /** CSS class name for styling the edge coordinate. */
    readonly className?: string;
    /** Orientation type of the coordinate, either vertical or horizontal. */
    readonly type: "vertical" | "horizontal";
    /** Coordinate value to display, typically a number or string. */
    readonly coordinate?: any;
    /** Starting x-coordinate for the coordinate line. */
    readonly x1: number;
    /** Starting y-coordinate for the coordinate line. */
    readonly y1: number;
    /** Ending x-coordinate for the coordinate line. */
    readonly x2: number;
    /** Ending y-coordinate for the coordinate line. */
    readonly y2: number;
    /** Position orientation of the coordinate label relative to the line. */
    readonly orient?: "bottom" | "top" | "left" | "right";
    /** Width of the coordinate label rectangle. */
    readonly rectWidth?: number;
    /** Whether to hide the coordinate line, showing only the label. */
    readonly hideLine?: boolean;
    /** Fill color for the coordinate label background. */
...
export class EdgeCoordinate extends React.Component<EdgeCoordinateProps> {
    public static defaultProps = {
        className: "reincharts-edgecoordinate",
        orient: "left",
        hideLine: false,
        fill: "#8a8a8a",
    ...
    public render() {
        ...
        if (edge.line !== undefined) {
            line = (
                <line
                    className="reincharts-cross-hair"
                ...
                edge.orient === "left" || edge.orient === "right" ? (
                    <g transform={`translate(${edge.coordinateBase.edgeXRect},${edge.coordinateBase.edgeYRect})`}>
                        <path
                            d={path}
                            key={1}
                            className="reincharts-text-background"
                            height={rectHeight}
                            width={rectWidth}
